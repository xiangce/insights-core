...
orig_hostname = socket.gethostname().split('.')[0]
...
client:
    ...
    persist:
        ...
        - name: insights.tests.test_specs_datasource.Specs
          enabled: true
...
plugins:
    ...
    packages:
        - insights.tests.test_specs_datasource
    ...
    configs:
        ...
        - name: insights.tests.test_specs_datasource.Specs
          enabled: true
        - name: insights.tests.test_specs_datasource.Stuff
          enabled: true
""".strip()
...
def test_specs_ds_with_hn_collect(mock_fun, obfuscate):
    ...
    for spec in Specs.__dict__:
        if not spec.startswith(('__', 'context_handlers', 'registry')):
            file_name = "insights.tests.test_specs_datasource.Specs.{0}.json".format(spec)
