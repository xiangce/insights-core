...
from insights.specs.datasources.httpd import (
    httpd_cmds,
    httpd_on_nfs,
    httpd_configuration_files,
    httpd24_scl_configuration_files,
    httpd24_scl_jbcs_configuration_files,
)
...
def test_httpd_conf_files_lower_case(m_open, m_glob, m_isdir, m_isfile):
    handlers = (
        m_open.return_value,
        mock_open(read_data=data_lines_ssl_conf_lower_case).return_value,
    )
    m_open.side_effect = handlers
    broker = {HostContext: None}
    result = httpd_configuration_files(broker)
...
def test_httpd24_scl_jbs_conf_files(m_open, m_glob, m_isdir, m_isfile):
    ...
    assert result == set(
        [
            '/opt/rh/jbcs-httpd24/root/etc/httpd/conf.d/ssl.conf',
